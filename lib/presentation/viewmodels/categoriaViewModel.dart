...
class CategoriaViewModel with ChangeNotifier {
  ...
  Future<void> cargarCategorias(context) async{
    ...
    try{
      ...
      if(respuesta.codigoHttp==200){
        if(respuesta.datos is List<CategoriaEntidad>) {
          ...
          showDialog(context: context, builder: (context){
            return DialogError(titulo: 'datos inconsistentes', mensaje: 'no se ha obtenido una lista de tipo entidad para cargar los datos', codigo: -1);
          });
        }
      }else
      if(respuesta.codigoHttp==204){
        _categorias = [];
        showDialog(context: context, builder: (context){
          return DialogError(titulo: 'Exito en la operacion', mensaje: 'Lastimosamente no se encontraron categorias en el sistema', codigo: 204);
        });
      }else {
        _categorias = [];
        showDialog(context: context, builder: (context) {
          return DialogError(titulo: 'No se pudo obtener datos',
      ...
      showDialog(context: context, builder: (context){
        return DialogError(titulo: 'excepcion no controlada', mensaje: e.toString(), codigo: -1);
      });
    }
  }
  ...
  guardarCategoria(CategoriaEntidad entidad, context) async{
    ...
    try{
      ...
      if(respuesta!=null){
        ...
        showDialog(context: context, builder: (context){
          return DialogError(titulo: 'Sin informacion', mensaje: 'El servidor no ha informado sobre el estado de la peticion', codigo: -1);
        });
      }
    }catch(e){
      Navigator.pushNamed(context, AppRutas.categorias);
      showDialog(context: context, builder: (context){
        return DialogError(titulo: 'Error inesperado al usar el servicio, excepcion no controlada', mensaje: e.toString(), codigo: -1);
      });
    }
  }
